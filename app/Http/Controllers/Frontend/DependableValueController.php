...
class DependableValueController extends Controller
{
    ...
    public function findLevelWiseClass(Request $request)
    {
        ...
        if (is_numeric($id) && $id != '') {
            $data = LevelWiseClass::select('id', 'class_name', 'value', 'academic_level_id')->where('academic_level_id', $request->id)->get();
        } else
            $data = '';
    ...
    public function findLastClass(Request $request)
    {
        $id = preg_replace('/[^a-z\d ]/i', '', $request->id);

        if (is_numeric($id) && $id != '') {

            $data =   LevelWiseClass::select('value')->where('id', $id)->first();

            // dd($data->value);

            $class_value = intval($data->value) - 1;
            if ($class_value < 0) $class_value = 0;

            $data = LevelWiseClass::select('id', 'class_name', 'value', 'academic_level_id')->where('value', $class_value)->get();
        } else
            $data = '';
    ...
    public function findBankName(Request $request)
    {
        ...
        if (is_numeric($id) && $id != '') {
            $data = BankingServiceProvider::select('id', 'name', 'known_as', 'bank_type_id')->where('bank_type_id', $request->id)->get();
        } else
            $data = 'Entered';
