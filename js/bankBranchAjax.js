...
$("#banking_type").on("change", function () {
    let bank_type_id = $(this).val();

    getBankName(bank_type_id);
});
...
function getBankName(id) {
    ...
    let bankNameURL = base_url + "/admission/getBankName" + "/" + id;

    let output = "";
    console.log(bankNameURL);
    $.get(bankNameURL)
        .always(function () {
            $("#bank_name_id").find("*").not("#nullValueOption").remove();
        })
        .done(function (data) {
            console.log(data);
...
function getBankBranches(dist_id, bank_id) {
    let base_url = window.location.origin;
    let bankBranchurl = base_url + "/admission/findBankBranchName";
...
$("#bank_branch_id").on("change", function () {
    ...
    let base_url = window.location.origin;
    let routingURL = base_url + "/admission/findBankRoutingNumber";
    let csrfToken = $('meta[name="csrf-token"]').attr("content");
